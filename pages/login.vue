      ...
      >
        ...
        >
          ...
          <circle
            ...
            :r="r"
          />
          <!-- Cross-thread connectors (slightly irregular) -->
          <path
      ...
      >
        ...
        >
          ...
          >
            <button
              type="button"
              class="font-comic text-xs underline underline-offset-2"
              style="color:var(--text-muted);"
            >
              Forgot password?
            </button>
...
// Support ?mode=login or ?mode=signup from navbar links
const mode = ref<'signup' | 'login'>(
  route.query.mode === 'login' ? 'login' : 'signup',
)
...
// Connecting cross-threads between rings (wavy bezier paths)
const webPaths = (() => {
  const paths: string[] = []
  ...
  rings.forEach((r) => {
    for (let i = 0; i < lineCount; i++) {
      const a1 = (i * Math.PI * 2) / lineCount
      const a2 = ((i + 1) * Math.PI * 2) / lineCount
      const x1 = 400 + Math.cos(a1) * r
      const y1 = 400 + Math.sin(a1) * r
      const x2 = 400 + Math.cos(a2) * r
      const y2 = 400 + Math.sin(a2) * r
      // Slight inward curve for natural web look
...
// ── Handlers (UI only — wire to nuxt-auth-utils later) ──
async function handleEmail() {
  ...
  // TODO: connect to /api/auth/login or /api/auth/register (nuxt-auth-utils)
...
function handleOAuth(provider: 'google' | 'github' | 'discord') {
  // TODO: navigateTo(`/api/auth/${provider}`, { external: true })
