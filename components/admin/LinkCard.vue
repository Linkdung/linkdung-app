<template>
  <div
    class="comic-panel p-3 flex items-start gap-3 transition-opacity duration-200"
    :class="{ 'opacity-40': !link.isVisible }"
  >
    <span
      class="w-5 h-5 flex-shrink-0 mt-2 border-2 border-black"
      :style="{ background: getLinkBg(link) }"
    />
    ...
    <div class="flex-1 min-w-0 space-y-1">
      ...
      <div>
        <input
          v-model="localTitle"
          class="input-comic text-sm py-1 font-display tracking-wide w-full"
      ...
      <div>
        <input
          v-model="localUrl"
          class="input-comic text-xs py-1 w-full"
      ...
      <div class="flex items-center gap-1.5 flex-wrap">
        <button
          ...
          :key="style.value"
          :title="style.label"
          class="w-4 h-4 border-2 border-black hover:scale-125 transition-transform"
          :style="{
            background: style.bg,
        ...
        <input
          ...
          class="w-4 h-4 border-1 border-black cursor-pointer p-0"
          :style="{
    ...
    <div class="flex flex-col gap-1 flex-shrink-0">
      <button
        class="btn-ghost px-2 py-1 text-base leading-none"
      ...
      <button
        ...
        @click="store.toggleLinkHighlight(link.id)"
      >
        <IconStar
          v-if="link.isHighlighted"
          :size="18"
          fill="#facc15"
          stroke="0"
        />
      ...
      <button
        class="btn-ghost px-2 py-1 text-base leading-none"
        @click="$emit('delete', link.id, link.title)"
      >
        <IconTrash2 :size="18" />
      </button>
    </div>
